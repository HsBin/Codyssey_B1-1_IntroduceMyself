...
네비게이션은 로고, 메뉴, 버튼과 같은 요소를 한 방향으로 정렬해야 하기 때문에
Flexbox를 사용했습니다.

Flexbox의 `justify-content`와 `align-items`를 활용하여
네비게이션 요소의 가로 정렬과 수직 정렬을 간단하게 구성했습니다.

### Grid

프로젝트 목록은 여러 개의 프로젝트 카드를 행과 열 형태로 배치해야 하기 때문에
CSS Grid를 사용했습니다.

`repeat(auto-fit, minmax(250px, 1fr))`을 사용하여 화면 크기에 따라
프로젝트 카드의 개수가 자동으로 조절되도록 구현했습니다.

이를 통해 모바일에서는 적은 수의 카드가 표시되고,
화면이 넓어질수록 한 행에 더 많은 카드가 배치되도록 구성했습니다.
